			...
			if (ret.success){
				...
				switch (message) {
					case 'Invalid session':
						message = TOP.TEXT.UPLOAD_INVALID_SESSION;
						break;
				}
 ...
 * @param {Object} params
 */
Tudu.EditorUpload = function(params) {
	this.init(params);
}

Tudu.EditorUpload.completes = 0;
Tudu.EditorUpload.fileCount = 0;

/**
 * 上传控件事件处理
 */
Tudu.EditorUpload.handlers = {
	/**
	 * 文件队列创建事件
	 */
	 ...
	 * 文件选择窗口关闭触发事件
	 */
	file_dialog_complete_handler: function(fileCount, queryLength) {
	},
	...
	upload_progress_handler : function(file, uploaded, total) {
		...
		try {
			var stats = this.getStats();
			Tudu.EditorUpload.completes = stats.upload_errors + stats.successful_uploads;
			Tudu.EditorUpload.fileCount = stats.files_queued + Attachment.completes;
			
			this.totalProgress = parseInt((((Tudu.EditorUpload.completes/EditorUpload.fileCount) + Tudu.EditorUpload.completes)/Tudu.EditorUpload.fileCount) * 100);
		} catch (e) {}
	},
 ...
 * 上传进度对话框
 */
Tudu.EditorUpload.dialog = null;
/**
 * 
 * @prototype
 * @class EditorUpload
 */
Tudu.EditorUpload.prototype = {
	...
	init: function(params) {
		if (typeof(params.onDialogComplete) == 'function') {
			this.onDialogComplete = params.onDialogComplete;
		}
		if (typeof(params.onComplete) == 'function') {
			this.onComplete = params.onComplete;
		}
		if (typeof(params.onFileQueue) == 'function') {
			this.onFileQueue = params.onFileQueue;
		}
		if (typeof(params.onFileComplete) == 'function') {
	 ...
	 */
	onStart: function(){
		if (null == Tudu.EditorUpload._dialog) {
			Tudu.EditorUpload._dialog = TOP.Frame.Dialog.show({
				body:  '<div>' + TOP.TEXT.FILE_UPLOADING + '</div><div class="progress_upload" style="height:14px;"></div>',
				title: TOP.TEXT.FILE_UPLOADING,
				close: false
			}).getWin();
		}
		
		Tudu.EditorUpload._dialog.show();
	},
	...
	setUpload: function(upload) {
		var _this = this;
		this._upload = upload;
		var handlers = $.extend(Tudu.EditorUpload.handlers, {
			file_dialog_start_handler: function(){
				var swfup = _this._upload.getSwfUploadObj();
				stats = swfup.getStats();
			...
			file_dialog_complete_handler: function(){
				_this._files = {};
				_this.onDialogComplete();
			},
			file_queued_handler: function(file){
				_this._files[file.id] = file;
				_this.onFileQueue(file);
			},
			upload_success_handler: function(file, response) {
				try {
					var ret;
					...
					if (ret.success){
						var fileid = ret.fileid ? ret.fileid : (ret.data ? ret.data.fileid : null);
						file.aid = fileid;
						_this._success.push(file);
					} else {
						getTop().showMessage(Format);
					}
			...
			upload_start_handler: this.onStart,
			upload_complete_handler: function(){
				try {
					var stats = this.getStats();
					_this.onFileComplete();
					
					if (!stats.files_queued && !stats.in_progress) {
						Tudu.EditorUpload._dialog.close();
						_this.onComplete();
						_this._success = [];
						return ;
					}
			...
			upload_progress_handler: function(file, uploaded, total) {
				var d = Tudu.EditorUpload._dialog.dialog.container;
				var bar = d.find('div.progress_large div.bar');
				
				var percent = parseInt(uploaded / total);
				
				try {
					var stats = this.getStats(),
					completes = stats.upload_errors + stats.successful_uploads,
					fileCount = stats.files_queued + completes;
					//alert(completes/fileCount);
					var progress = Math.min(95, parseInt(((completes/fileCount) + percent/fileCount) * 100));
					//alert(progress);
					bar.css('width', progress + '%');
				} catch (e) {}
			}
		});
		this._upload.setHandler(handlers);
	},
...
TuduUpload.prototype = {
	 ...
	 */
	init: function() {
		var params = $.extend({}, TuduUpload.params), key;
		
		for (var k in this.opts) {
			if (k.indexOf('on') == 0 && typeof(this[k]) == 'function') {
				this[k] = this.opts[k];
				continue;
			}
		...
		//alert(TuduUpload.params.file_types);
		try {
	...
	totalProgress: function() {
		...
		return this._swfupload.totalProgress ? this._swfupload.totalProgress : 0;
	},
...
TuduUpload.defaults = {
	flashUrl: TOP._FILECGI.swfupload,
	uploadUrl: TOP._FILECGI.upload,
