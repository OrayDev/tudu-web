...
function Datepick() {
	...
	this._defaults = { // Global defaults for all the date picker instances
		...
		// modified by CuTe_CuBe 
		showButtonPannel: false,
		showTimePanel: false
	};
...
$.extend(Datepick.prototype, {
	...
	_showDatepick: function(input) {
		input = input.target || input;//alert($.datepick._isDisabledDatepick(input));
		...
		// 2010-9-15
		// width fit to input element
		var inp = $(input), 
			inpBorder = $.datepick._getBorders(inp),
			inpWidth = inp.outerWidth() - inpBorder[0] - inpBorder[1];
		
		inst.dpDiv.width(Math.max(180, inpWidth));
	...
	_selectDay: function(td, id, timestamp) {
		...
		var buttonPanel = this._get(inst, 'showButtonPanel');
		if (rangeSelect)
			inst.stayOpen = !inst.stayOpen;
		else if (multiSelect || buttonPanel)
			inst.stayOpen = true;
		if (inst.stayOpen) {
	...
	_showDate: function(inst) {
		...
		if (inst.input) {
			...
			if (dateStr && this._get(inst, 'showTimePanel')) {
				var hour = $('#datepick-div').find('input:text[name="hour"]').val(),
					min  = $('#datepick-div').find('input:text[name="minute"]').val();
	...
	_generateHTML: function(inst) {
		...
		var timePanel = this._get(inst, 'showTimePanel') ? this._generateTimePanel(inst) : '';
		var buttonPanel = this._get(inst, 'showButtonPanel') ? this._generateButtonPanel(inst) : '';
		...
		var html = (closeAtTop && !inst.inline ? controls : '') +
			...
			'<a href="javascript:void(0)" onclick="jQuery.datepick._selectDay(this, \'#' + inst.id + '\', ' + new Date().getTime() +');"' +
		...
		html += (showStatus ? '<div style="clear: both;"></div><div id="' + this._statusId[useTR] +
			...
			'<div style="clear: both;"></div>' + timePanel + buttonPanel + 
	 ...
	 * 生成时间填写框
	 */
	_generateTimePanel: function(inst) {
		return '<div class="datepick-time-panel">' + this._get(inst, 'timeText') + ' : ' + 
			'<input type="text" class="datepick-time-input" value="" name="hour" maxlength="2" onkeyup="var h = parseInt(this.value);if (!h || h > 23 || h < 0) {this.value = \'\';}else{this.value=h}" /> : ' + 
			'<input type="text" class="datepick-time-input" value="" name="minute" maxlength="2" onkeyup="var m = parseInt(this.value);if (!m || m > 60 || m < 0) {this.value = \'\';}else{this.value=m}" /></div>';
	},
	
	/**
	 * 生成底部按钮
	 */
	_generateButtonPanel: function(inst) {
		return '<div class="datepick-button-panel">' + 
		'<input type="button" class="datepick-button" value="' + this._get(inst, 'confirmSelect') + '" onclick="jQuery.datepick._confirmInput(\'' + inst.id + '\')" /></div>';
	},
	
	/**
	 * 确定选择
	 */
	_confirmInput: function(id) {
		var inst = $.datepick._getInst($('#' + id)[0]);
		inst.stayOpen = false;
		this._updateInput('#' + id);
	},
