...
{{math equation="650/y" y=$headers|@count assign=tdwidth}}
{{strip}}
...
{{if $isquery || ($label && $label.labelalias == 'inbox' || $label.labelalias == 'todo')}}
{{*循环分组*}}
{{foreach key=key item=group from=$tudus}}
{{if $key != 'discuss' && $key != 'notice'}}
<div id="tudu-list" class="gantt_list_wrap">
...
<div class="grid_list_title"><span class="icon icon_elbow_minus toggle_tudu"></span><h3>{{$LANG.me}}</h3> (<span class="grid_list_count">{{$tudus|@count}}</span>)</div>
<div class="grid_list_group_ct">
{{*循环列表显示*}}
