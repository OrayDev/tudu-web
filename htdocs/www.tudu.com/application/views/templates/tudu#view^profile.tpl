<table width="100%" border="0" cellspacing="0" cellpadding="0">
  ...
  {{else}}
    <td valign="top" width="50"><div class="list_avatar"><a href="javascript:void(0)" title="{{$LANG.user_chat}}"><img _email="{{$post.email}}"{{if $post.uniqueid == '^system'}} _name="{{$post.poster}}"{{/if}} _url="/contact/view?email={{$post.email}}&back={{$currUrl}}" class="todo_send_icon" src="/logo?unid={{$post.uniqueid}}"></a></div></td>
