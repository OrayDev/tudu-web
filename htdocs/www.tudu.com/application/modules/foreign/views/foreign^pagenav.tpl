...
{{if $pagenav.pagecount > 1}}&nbsp;<input type="text" name="pageinput" class="input_text" style="width:20px;text-align:center" /><button name="gopage" type="button" style="margin:0" onclick="location=('{{page_url url=$pagenav.url page='$page' query=$pagenav.query}}').replace('%24page', $(this).prev().val())">Go</button>{{/if}}
</div>
{{/strip}}