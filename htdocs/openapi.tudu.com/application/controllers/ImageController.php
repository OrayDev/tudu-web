<?php
/**
 * Tudu OpenApi
 *
 * LICENSE
 *
 *
 * @copyright  Copyright (c) 2009-2010 Shanghai Best Oray Information S&T CO., Ltd.
 * @link       http://www.oray.com/
 * @version    $Id$
 */


/**
 * @copyright  Copyright (c) 2009-2010 Shanghai Best Oray Information S&T CO., Ltd.
 */
class ImageController extends TuduX_Controller_OpenApi
{

    const FORMAT_DEFAULT = 'json';
    const FORMAT_BINARY  = 'binary';
    const FORMAT_BASE64  = 'base64';

    /**
     *
     * @var binary
     */
    private $_image = null;

    /**
     *
     * @var string
     */
    private $_mimeType = null;

    /**
     * (non-PHPdoc)
     * @see Zend_Controller_Action::preDispatch()
     */
    public function preDispatch()
    {
        $action = $this->_request->getActionName();

        // 用户未登录
        if (in_array($action, array('avatar', 'attach', 'attach-thumb')) && !$this->_user->isLogined()) {
            throw new TuduX_OpenApi_Exception("Access protected API without a valided access token", TuduX_OpenApi_ResponseCode::MISSING_AUTHORIZE, 403);
        }
    }

    /**
     * 获取企业logo
     */
    public function logoAction()
    {
        $orgId  = $this->_request->getQuery('orgid');

        /* @var $orgDao Dao_Md_Org_Org */
        $orgDao = Tudu_Dao_Manager::getDao('Dao_Md_Org_Org', Tudu_Dao_Manager::DB_MD);
        $logo   = $orgDao->getLogo($orgId);

        if (!$logo) {
            $logo = base64_decode('');
        }

        $this->_image = $logo;
    }

    /**
     * 获取用户/联系人头像
     */
    public function avatarsAction()
    {
        $contactId = $this->_request->getQuery('contactid');

        // 图度用户头像
        if (strpos($contactId, '@')) {

            /* @var $userDao Dao_Md_User_User */
            $userDao = Tudu_Dao_Manager::getDao('Dao_Md_User_User', Tudu_Dao_Manager::DB_MD);

            list($userId, $orgId) = explode('@', $contactId, 2);

            $avatars = $userDao->getAvatars(array('userid' => $userId, 'orgid' => $orgId));

            if (empty($avatars['avatars'])) {
                $this->_mimeType = 'image/png';
                if (!isset($avatars['gender']) || $avatars['gender']) {
                    $this->_image = base64_decode('');
                } else {
                    $this->_image = base64_decode('');
                }
            } else {
                $this->_mimeType = $avatars['type'];
                $this->_image    = $avatars['avatars'];
            }

        // 个人联系人头衔
        } else {

            /* @var $daoContact Dao_Td_Contact_Contact */
            $daoContact = Tudu_Dao_Manager::getDao('Dao_Td_Contact_Contact', Tudu_Dao_Manager::DB_TS);

            $info = $daoContact->getAvatars($contactId);

            if (null !== $info) {
                $this->_image    = $info['avatars'];
                $this->_mimeType = $info['avatarstype'];
            }
        }

        if (empty($this->_image)) {
            $this->_mimeType = 'image/png';
            $this->_image    = base64_decode('');
        }
    }

    /**
     * 附件图片缩略图获取接口
     */
    public function thumbAction()
    {
        $url  = $this->_request->getQuery('img');
        $size = $this->_request->getQuery('size');

        $width  = null;
        $height = null;
        if ($size) {
            $size = str_replace('*', 'x', $size);
            $arr = explode('x', strtolower($size));
            $width = (int)$arr[0];
            $height = (int) (isset($arr[1]) ? $arr[1] : $width);
        }

        // 附件
        if (strpos($url, 'AID:') === 0) {
            $fileId = substr($url, 4);

            $this->_getAttachmentThumb($fileId, $width, $height);
        } else {
            $this->_getRemoteFileThumb($url, $width, $height);
        }
    }

    /**
     *
     * @param string $url
     * @param int $width
     * @param int $height
     */
    private function _getRemoteFileThumb($url, $width, $height)
    {
        $content = file_get_contents($url);

        if (!$content) {
            throw new TuduX_OpenApi_Exception('Could not read image from remote server', TuduX_OpenApi_ResponseCode::SYSTEM_ERROR);
        }

        $img  = imagecreatefromstring($content);

        if (!$img) {
            throw new TuduX_OpenApi_Exception('Invalid image format', TuduX_OpenApi_ResponseCode::SYSTEM_ERROR);
        }

        if (null === $width || null === $height) {
            ob_start();
            imagejpeg($img);
            $data = ob_get_contents();
            ob_end_clean();

            $this->_image    = $data;
            $this->_mimeType = 'image/jpeg';
            return ;
        }

        $sw = imagesx($img);
        $sh = imagesy($img);

        $thumb = imagecreatetruecolor($width, $height);
        imagefill($thumb, 0, 0, imagecolorallocate($thumb, 255, 255, 255));

        // 坐标映射
        if ($sw / $sh == $width / $height) {
            $distW = $width;
            $distH = $height;
            $distX = 0;
            $distY = 0;
        } else {

            if ($sw >= $sh) {
                $distW = $width;
                $distH = $sh * ($distW / $sw);
                $distX = 0;
                $distY = ($height - $distH) / 2;
            } else {
                $distH = $height;
                $distW = $sw * ($distH / $sh);
                $distX = ($width - $distW) / 2;
                $distY = 0;
            }
        }

        imagecopyresized($thumb, $img, $distX, $distY, 0, 0, $distW, $distH, $sw, $sh);

        imagedestroy($img);

        ob_start();
        imagejpeg($thumb);
        $data = ob_get_contents();
        ob_end_clean();

        $this->_image    = $data;
        $this->_mimeType = 'image/jpeg';
    }

    /**
     *
     * @param unknown_type $fileId
     */
    private function _getAttachmentThumb($fileId, $width, $height)
    {
        $path = $this->_bootstrap->getOption('path');

        /* @var $daoFile Dao_Td_Attachment_File*/
        $daoFile = Tudu_Dao_Manager::getDao('Dao_Td_Attachment_File', Tudu_Dao_Manager::DB_TS);

        $file = $daoFile->getFile(array('fileid' => $fileId));

        if (null == $file) {
            throw new TuduX_OpenApi_Exception('Images is not exists', TuduX_OpenApi_ResponseCode::RESOURCE_NOT_EXISTS);
        }

        //$fileName = $path['attachment'] . '/' . $file->path . '/' . $file->fileId;
        $sId      = 'ACCESS-TOKEN-' . $this->_accessToken;
        $fileName = 'http://upload.tudu.com/orayfile/download?sid=' . $sId . '&fid=' . $fileId . '&auth=' . md5($sId . $fileId . $this->_token['auth']['logintime']);

        /*if (!file_exists($fileName)) {
            throw new TuduX_OpenApi_Exception('Images is not exists', TuduX_OpenApi_ResponseCode::RESOURCE_NOT_EXISTS);
        }*/

        //if (null === $width || null === $height) {
            $data = file_get_contents($fileName);

            $this->_image    = $data;
            $this->_mimeType = 'image/jpeg';
            //return ;
        //}

        if (null === $width || null === $height) {
            return ;
        }
/*
        $type = null;
        if (0 === strpos($file->type, 'image/')) {
            $type = strtolower(str_replace('image/', '', $file->type));
            $type = $type == 'jpg' ? $type = 'jpeg' : $type;
        } else {
            $imgInfo = getimagesize($fileName);

            if ($imgInfo) {
                $type = strtolower(str_replace('image/', '', $imgInfo['mime']));
            }
        }

        $type = $type == 'jpg' ? $type = 'jpeg' : $type;
        if (!in_array($type, array('jpeg', 'jpg', 'gif', 'png', 'bmp'))) {
            throw new TuduX_OpenApi_Exception('Images is not exists', TuduX_OpenApi_ResponseCode::RESOURCE_NOT_EXISTS);
        }
*/
        // 权限
        //if () {}

        //$func = 'imagecreatefrom' . $type;
        $func = 'imagecreatefromstring';
        $img  = $func($data);

        if (!$img) {
            throw new TuduX_OpenApi_Exception('Could not read image file', TuduX_OpenApi_ResponseCode::SYSTEM_ERROR);
        }

        $sw = imagesx($img);
        $sh = imagesy($img);

        $thumb = imagecreatetruecolor($width, $height);
        imagefill($thumb, 0, 0, imagecolorallocate($thumb, 255, 255, 255));

        // 坐标映射
        if ($sw / $sh == $width / $height) {
            $distW = $width;
            $distH = $height;
            $distX = 0;
            $distY = 0;
        } else {

            if ($sw >= $sh) {
                $distW = $width;
                $distH = $sh * ($distW / $sw);
                $distX = 0;
                $distY = ($height - $distH) / 2;
            } else {
                $distH = $height;
                $distW = $sw * ($distH / $sh);
                $distX = ($width - $distW) / 2;
                $distY = 0;
            }
        }

        imagecopyresized($thumb, $img, $distX, $distY, 0, 0, $distW, $distH, $sw, $sh);

        imagedestroy($img);

        ob_start();
        imagejpeg($thumb);
        $data = ob_get_contents();
        ob_end_clean();

        $this->_image    = $data;
        $this->_mimeType = 'image/jpeg';
    }

    private function _getMimeType($bin)
    {
        if (null !== $this->_mimeType) {
            return $this->_mimeType;
        }

        $strInfo = @unpack("C2chars", $bin);
        $typeCode = intval($strInfo['chars1'].$strInfo['chars2']);
        $fileType = '';
        switch ($typeCode) {
            case 255216:
                $fileType = 'image/jpeg';
                break;
            case 7173:
                $fileType = 'image/gif';
                break;
            case 6677:
                $fileType = 'image/bmp';
                break;
            case 13780:
                $fileType = 'image/png';
                break;
            default:
                break;
        }

        return $fileType;
    }

    /**
     * (non-PHPdoc)
     * @see Zend_Controller_Action::postDispatch()
     */
    public function postDispatch()
    {
        $format = $this->_request->getQuery('format', self::FORMAT_DEFAULT);

        switch ($format) {
            case self::FORMAT_BINARY:
                $this->_helper->viewRenderer->setNeverRender();

                $this->_response->setHeader('Expires', gmdate('D, d M Y H:i:s T', time() + 36000), true);
                $this->_response->setHeader('Cache-Control', 'private', true);
                $this->_response->setHeader('Pragma', 'private', true);
                $this->_response->setHeader('Content-type', $this->_getMimeType($this->_image));
                $this->_response->setHeader('Content-Length', strlen($this->_image));

                echo $this->_image;

                break;
            case self::FORMAT_BASE64:
                $this->_helper->viewRenderer->setNeverRender();
                echo base64_encode($this->_image);

                break;

            case self::FORMAT_DEFAULT:
            default:
                $this->view->image = base64_encode($this->_image);
                $this->view->mime  = $this->_mimeType;
                parent::postDispatch();
                break;
        }
    }
}