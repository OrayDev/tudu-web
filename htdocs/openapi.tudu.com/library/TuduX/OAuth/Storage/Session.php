 ...
 * @copyright  Copyright (c) 2009-2010 Shanghai Best Oray Information S&T CO., Ltd.
 */
class TuduX_OAuth_Storage_Session implements OpenApi_OAuth_Storage_Interface
{
    ...
    public function setAccessToken($accessToken, array $data, $timeout = null)
    {
        $save = array(
            'token' => array(
                OpenApi_OAuth_OAuth::PARAM_CLIENT_ID => $data[OpenApi_OAuth_OAuth::PARAM_CLIENT_ID],
                OpenApi_OAuth_OAuth::PARAM_USER_ID   => $data[OpenApi_OAuth_OAuth::PARAM_USER_ID],
                OpenApi_OAuth_OAuth::PARAM_EXPIRES   => $data[OpenApi_OAuth_OAuth::PARAM_EXPIRES],
                OpenApi_OAuth_OAuth::PARAM_SCOPE     => $data[OpenApi_OAuth_OAuth::PARAM_SCOPE]
            )
        );
        ...
        $save = self::SESSION_NAMESPACE . '|' . serialize($save);

        return $this->_memcache->set($this->_makeAccessTokenKey($accessToken), $save, null, $timeout, true);
    }
    ...
    {
        $flag = null;
        $data = $this->_memcache->get($this->_makeAccessTokenKey($accessToken), $flag, true);

        $data = @unserialize(str_replace(self::SESSION_NAMESPACE . '|', '', $data));
