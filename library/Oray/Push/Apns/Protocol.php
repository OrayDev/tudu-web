...
class Oray_Push_Apns_Protocol
{
    ...
    public function connect()
    {
        ...
        if ($err) {
            ...
            throw new Oray_Push_Exception("Connect to apple push server failed with  error: CODE[{$err}] {$errstr};");
        }
    ...
    public function push($deviceToken, array $playload, $expires = 0)
    {
        ...
        if (!$result) {
            require_once 'Oray/Push/Exception.php';
            throw new Oray_Push_Exception("Connect to apple push server failed", -1);
        }
        ...
        if (false === $numChanged) {
            require_once 'Oray/Push/Exception.php';
            throw new Oray_Push_Exception("Failed selecting stream to read.", -1);
        }
        ...
        if ($numChanged > 0) {
            ...
            if($status>0) {
                ...
                require_once 'Oray/Push/Exception.php';
                throw new Oray_Push_Exception("APNS responded with error for pid($identifier). status($status: $desc)", $status);

                $this->close();
            }

            require_once 'Oray/Push/Exception.php';
            throw new Oray_Push_Exception("APNS responded with command($command) status($status) pid($identifier).", $status);
        }
    ...
    public function feedback()
    {
        if (empty($this->_options['feedback'])) {
            require_once 'Oray/Push/Exception.php';
            throw new Oray_Push_Exception('Undefined host for feedback service');
        }
        ...
        if (!$fp) {
            require_once 'Oray/Push/Exception.php';
            throw new Oray_Push_Exception("Failed to connect to feedback service. with error: {$errStr}, code: {$err}");
        }
    ...
    {
        require_once 'Oray/Push/Apns/Message.php';
        return new Oray_Push_Apns_Message();
    }
